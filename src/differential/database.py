...
class DB:
    def __init__(
        self, client: Client | None = None, schema: str = 'public',
    ):
    ...
    def update_call_status(
        ...
        completed_at = (
            ...
            if status == CallStatus.COMPLETE else None
        )
    ...
    def save_page_rating(
        self, page_id: str, call_id: str, score: int, note: str = '',
    ) -> None:
    ...
    def count_pages_for_question(self, question_id: str) -> dict:
        ...
        judgements_result = self.client.rpc(
            'count_active_judgements', {'qid': question_id},
        ).execute()
        return {
